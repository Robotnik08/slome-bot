...
bot.on('guildMemberAdd', member => {
    // give them the role
    member.roles.add('1164913811573186590');
});
...
bot.on("messageCreate", async message => {
    ...
    if(!cmd.startsWith(prefix)) {
        ...
        if ((message.channel.name === "slome-bot" && message.mentions.users.first()) || Math.random() < 0.01) { // 1% chance of responding to any message anywhere
